...
def run_tests():
    ...
    for i, mixing in enumerate(mixings):
        ...
        try:
            ...
            print(
                f"Passed α = {round(mixing,4)}, KRR error = ({krr_errors[i]}), KPCA error = ({kpca_errors[i]})")
        except AssertionError:
            return False

    try:
        assert all(krr_errors[i]-tolerance <= krr_errors[i+1] and kpca_errors[i]
                   >= kpca_errors[i+1]-tolerance for i in range(len(krr_errors)-1))
    except AssertionError:
        for i, mixing in enumerate(mixings[:-1]):
            if(krr_errors[i]-tolerance > krr_errors[i+1]):
                print(
                    f"KRR error at α = {mixing} ({krr_errors[i]}) > α = {mixings[i+1]} ({krr_errors[i+1]})")
            if(kpca_errors[i] < kpca_errors[i+1]-tolerance):
                print(
                    f"KPCA error at α = {mixing} ({kpca_errors[i]}) < α = {mixings[i+1]} ({kpca_errors[i+1]})")
    ...
    for i, mixing in enumerate(mixings):
        ...
        krr_errors[i] = np.linalg.norm(Y-Yp)**2.0 / np.linalg.norm(Y)**2.0
        kpca_errors[i] = np.linalg.norm(
            K-np.matmul(T, T.T))**2.0 / np.linalg.norm(K)**2.0
