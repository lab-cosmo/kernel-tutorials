...
class KPCovR:
    ...
    def __init__(self, alpha=0.0, n_PC=2, kernel_type="linear", regularization=1e-12):
    ...
    def fit(self, X, Y, K=None, Yhat=None):
        ...
        self.PKT = np.matmul(P, np.matmul(self.U[:, : self.n_PC], np.sqrt(v_inv)))
    ...
    def statistics(self, X, Y, K=None):
        ...
        return get_stats(x=X, y=Y, yp=Yp, t=Tp, xr=Xr, k=K, kapprox=Kapprox)
...
class SparseKPCovR:
    ...
    def __init__(
        self, alpha, n_PC, n_active=100, regularization=1e-6, kernel_type="linear"
    ):
