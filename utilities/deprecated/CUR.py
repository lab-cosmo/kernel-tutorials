...
class CUR:
    ...
    def __init__(
        ...
        self.A = matrix
        self.symmetric = (
            self.A.shape == self.A.T.shape
            and np.all(np.abs(self.A - self.A.T)) < symmetry_tolerance
        )
    ...
    def compute(self, n_c, n_r=None):
        ...
        # Compute S.
        S = np.matmul(np.matmul(np.linalg.pinv(A_c), self.A), np.linalg.pinv(A_r))
        return A_c, S, A_r
    ...
    def loss(self, n_c, n_r=None):
        ...
        return np.linalg.norm(
            self.A - np.matmul(A_c, np.matmul(S, A_r))
