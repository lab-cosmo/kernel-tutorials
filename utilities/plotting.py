...
def plot_loss(X, Y, alphas, components, fig=None, axs=None):
    ...
    if len(axs) != 2:
        print("Must supply two axes objects")
        return
    ...
    for cdx, c in enumerate(components):
        for adx, a in enumerate(alphas):
            pcovr_alpha = PCovR(alpha=a, n_PCA=c)
            pcovr_alpha.fit(X, Y)
            L_pca[cdx, adx], L_lr[cdx, adx] = pcovr_alpha.loss(X, Y)

    for cdx, c in enumerate(components):
        axs[0].loglog(L_lr[cdx, :], L_pca[cdx, :],
                      marker='o', label='{:d} PCs'.format(c))

        loss_sum = L_lr[cdx, :] + L_pca[cdx, :]
        axs[1].semilogy(alphas, loss_sum, marker='o',
                        label='{:d} PCs'.format(c))
        print('Optimal alpha for {:d} PCs = {:.2f}'.format(
            c, alphas[np.argmin(loss_sum)]))

    axs[0].set_xlabel(r'$L_{LR}$')
    axs[0].set_ylabel(r'$L_{PCA}$')
    axs[0].legend()

    axs[1].set_xlabel(r'$\alpha$')
    axs[1].set_ylabel(r'$L_{LR} + L_{PCA}$')
    axs[1].legend()
