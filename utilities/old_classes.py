...
class KPCA:
    ...
    def fit(self, X=None, K=None):
        ...
        self.v, self.U = sorted_eig(K, thresh=self.regularization, n=self.n_PC)

        v_inv = eig_inv(self.v[: self.n_PC])

        self.PKT = np.matmul(self.U[:, : self.n_PC], np.diagflat(np.sqrt(v_inv)))
        self.T = np.matmul(K, self.PKT)
        self.PTK = np.matmul(np.diagflat(v_inv), np.matmul(self.T.T, K))
        if X is not None:
            self.PTX = np.matmul(np.diagflat(v_inv), np.matmul(self.T.T, X))
...
class SparseKPCA:
    ...
    def __init__(self, n_PC, n_active=100, regularization=1e-12, kernel_type="linear"):
    ...
    def fit(self, X, i_sparse=None, X_sparse=None, Kmm=None, Knm=None):
        ...
        self.v_C, self.U_C = sorted_eig(C, thresh=self.regularization, n=self.n_active)

        self.PKT = np.matmul(self.U_active, self.U_C[:, : self.n_PC])
        self.T = np.matmul(Knm - self.barKM, self.PKT)
        self.PTX = np.matmul(
            np.diagflat(eig_inv(self.v_C[: self.n_PC])), np.matmul(self.T.T, X)
        )
...
class SparseKRR:
    ...
    def __init__(self, regularization=1.0e-16, n_active=100, kernel_type="linear"):
...
class PCovR:
    ...
    def fit_feature_space(self, X, Y, Yhat=None):
        ...
        iCsqrt = np.matmul(np.matmul(U_C, np.diagflat(np.sqrt(eig_inv(v_C)))), U_C.T)
        ...
        self.PXT = np.matmul(PXV, np.diagflat(np.sqrt(v_Ct[: self.n_PC])))
        self.PTX = np.matmul(
            np.diagflat(np.sqrt(v_inv)), np.matmul(U_Ct[:, : self.n_PC].T, Csqrt)
        )
        PTY = np.matmul(np.diagflat(np.sqrt(v_inv)), U_Ct[:, : self.n_PC].T)
    ...
    def fit_structure_space(self, X, Y, Yhat=None):
        ...
        self.compute_K(X, Y, Yhat=Yhat)
        self.v, self.U = sorted_eig(self.K, thresh=self.regularization, n=self.n_PC)

        v_inv = eig_inv(self.v[: self.n_PC])
        self.T = np.matmul(
            self.U[:, : self.n_PC], np.diagflat(np.sqrt(self.v[: self.n_PC]))
        )
        ...
        self.PXT = np.matmul(
            P, np.matmul(self.U[:, : self.n_PC], np.diag(np.sqrt(v_inv)))
        )
        self.PTY = np.matmul(np.diagflat(v_inv), np.matmul(self.T.T, Y))
