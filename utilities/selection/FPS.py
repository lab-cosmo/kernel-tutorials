...
class _FPS(_BaseSelection):
    ...
    def __init__(self, matrix, alpha=1.0, idxs=None, tolerance=1E-12, **kwargs):
...
class sampleFPS(_FPS):
    def __init__(self, matrix, alpha=1.0, tolerance=1E-12, **kwargs):
        super().__init__(matrix=matrix, alpha=alpha, tolerance=tolerance, **kwargs)
    ...
    def get_product(self):
        ...
        if(self.alpha < 1):
            K += (1 - self.alpha) * self.Y @ self.Y.T
        if(self.alpha > 0):
            K += (self.alpha) * self.A @ self.A.T
