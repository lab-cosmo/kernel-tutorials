    ...
    def transform(self, X, y=None):
        ...
        if self.n_samples_seen_ == 0:
            raise sk.exceptions.NotFittedError(
                "This "+type(self).__name__+" instance is not fitted yet. Call 'fit' with appropriate arguments before using this estimator.")
    ...
    def fit(self, K, y=None, K_active=None):
        ...
        if K_active is not None:
            ...
            if K_active.shape[0] != K_active.shape[1]:
                raise ValueError(
                    "K_active should be the square, active set kernel. ")

        self.n_samples_seen_ = K.shape[0]
        if self.__sparse_kernel:
            if K.shape[1] != self.n_active_:
                raise ValueError(
                    "For sparse kernels, K should be train-active set kernel. ")
    ...
    def transform(self, K_test, y=None, K_test_train=None):
        ...
        if self.__sparse_kernel:
            if K_test.shape[1] != self.n_active_:
                raise ValueError(
                    "Test kernel must have n_active as second dimension")
            if self.__with_mean:
            ...
            if K_test_train is None:
                if K_test.shape[1] != self.n_samples_seen_:
                    raise ValueError(
                        "Test kernel must have n_train as second dimension")
                if self.__with_mean:
                    K_std += self.K_train_mean_ - \
                        np.add.outer(K_test.mean(axis=1),
                                     self.K_train_row_mean_)
            else:
                K_rows, K_cols = K_test_train
                K_std += self.K_train_mean_ - \
                    np.add.outer(K_rows.mean(axis=1), K_cols.mean(axis=0))
