...
class KernelNormalizer(TransformerMixin, BaseEstimator):
    ...
    def transform(self, K, y=None, K_Atrain_and_Btrain = None):
        ...
        if self.__sparse_kernel:
            ...
            if K_Atrain_and_Btrain is None:
                if K.shape[1] != self.n_samples_seen_:
                    raise ValueError("Test kernel must have n_train as second dimension")
                if self.__with_mean:
                    K_std += self.K_train_mean_ - np.add.outer(K.mean(axis=1), self.K_train_row_mean_)
            else:
                K_rows, K_cols = K_Atrain_and_Btrain
